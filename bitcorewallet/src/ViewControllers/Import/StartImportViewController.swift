...
class StartImportViewController : UIViewController {
    ...
    init(walletManager: WalletManager) {
        ...
        assert(walletManager.currency is Bitcore, "Importing only supports Bitcore")
    ...
    private func handleData(data: [[String: Any]], key: BRKey) {
        ...
        guard let address = key.address() else { return }
        guard let fees = Currencies.btx.state.fees else { return }
        ...
        balanceActivity.dismiss(animated: true, completion: {
            ...
            let balanceAmount = Amount(amount: balance, rate: rate, maxDigits: Currencies.btx.state.maxDigits, currency: Currencies.btx)
            let feeAmount = Amount(amount: fee, rate: rate, maxDigits: Currencies.btx.state.maxDigits, currency: Currencies.btx)
    ...
    private func publish(tx: UnsafeMutablePointer<BRTransaction>, balance: UInt64, fee: UInt64, key: BRKey) {
        ...
        present(importingActivity, animated: true, completion: {
            ...
            var keys = [key]
            let _ = tx.sign(forkId: (self.currency as! Bitcore).forkId, keys: &keys)
