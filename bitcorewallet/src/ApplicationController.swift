...
class ApplicationController : Subscriber, Trackable {
    ...
    private var primaryWalletManager: WalletManager? {
        return walletManagers[Currencies.btx.code]
    }
    ...
    private func initWallet(currency: CurrencyDef, dispatchGroup: DispatchGroup) {
        dispatchGroup.enter()
        guard let currency = currency as? Bitcore else { return }
        ...
        walletManager.initWallet { success in
            guard success else {
                // always keep BTX wallet manager, even if not initialized, since it the primaryWalletManager and needed for onboarding
                if !currency.matches(Currencies.btx) {
...
//        For when watch app launches app in background
        if primaryWalletManager.noWallet {
            ...
            for (currencyCode, exchangeUpdater) in exchangeUpdaters {
                exchangeUpdater.refresh {
                    if currencyCode == Currencies.btx.code {
                        self.watchSessionManager.walletManager = self.primaryWalletManager
                        self.watchSessionManager.rate = Currencies.btx.state.currentRate
                    }
                }
            }
        }
    ...
    private func setupRootViewController() {
        let home = HomeScreenViewController(primaryWalletManager: walletManagers[Currencies.btx.code])
