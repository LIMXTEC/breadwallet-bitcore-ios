...
class ModalPresenter : Subscriber, Trackable {
    ...
    init(walletManagers: [String: WalletManager], window: UIWindow, apiClient: BRAPIClient) {
        ...
        self.primaryWalletManager = walletManagers[Currencies.btx.code]!
...
//        let url = articleId == nil ? "/support?" : "/support/\(articleId!).html"
    ...
    private func rootModalViewController(_ type: RootModal) -> UIViewController? {
        ...
        case .loginAddress:
            return receiveView(currency: Currencies.btx, isRequestAmountVisible: false)
    ...
    private func presentLoginScan() {
        ...
        let present = presentScan(parent: top, currency: Currencies.btx)
        Store.perform(action: RootModalActions.Present(modal: .none))
        present({ paymentRequest in
            guard let request = paymentRequest else { return }
            self.currentRequest = request
            self.presentModal(.send(currency: Currencies.btx))
        })
    }
        ...
        let rows = [
            ...
            SettingsSections.preferences: [
                Setting(title: LAContext.biometricType() == .face ? S.Settings.faceIdLimit : S.Settings.touchIdLimit, accessoryText: {
                    guard let rate = Currencies.btx.state.currentRate else { return "" }
                    let amount = Amount(amount: walletManager.spendingLimit, rate: rate, maxDigits: Currencies.btx.state.maxDigits, currency: Currencies.btx)
            ...
            if let topVC = topViewController as? ModalViewController {
                ...
                if !attemptConfirmRequest() {
                    ...
                    topVC.dismiss(animated: true, completion: {
                        Store.perform(action: RootModalActions.Present(modal: .send(currency: Currencies.btx)))
