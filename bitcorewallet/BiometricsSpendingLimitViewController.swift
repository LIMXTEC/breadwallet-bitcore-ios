...
class BiometricsSpendingLimitViewController: UITableViewController, Subscriber {
    ...
    override func viewDidLoad() {
        ...
        if !limits.contains(walletManager.spendingLimit) {
            if let rate = Currencies.btx.state.currentRate {
                let spendingLimit = Amount(amount: walletManager.spendingLimit, rate: rate, maxDigits: Currencies.btx.state.maxDigits, currency: Currencies.btx)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if limit == 0 {
            ...
            let displayAmount = DisplayAmount(amount: Satoshis(rawValue: limit), selectedRate: nil, minimumFractionDigits: 0, currency: Currencies.btx)
